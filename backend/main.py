...
from system import (
    ...
    get_firewall_status, firewall_enable, firewall_open_port, firewall_close_port,
    get_panel_port, change_panel_port,
    get_backup_settings, save_backup_settings, run_backup_now, list_backups, delete_backup,
    get_current_version, check_latest_version, upgrade_from_zip, rollback_version, list_backup_versions,  # v1.3.39 在线升级
    get_server_info,  # v1.3.43.x 服务器 IP/主机名/OS/内核/公网 IP/运行时间
)
from file_manager import list_directory, read_file, write_file, upload_file, delete_file, chmod_file, create_directory, extract_archive
...
def api_databases_create():
    ...
    _sync_pma_bridge(db_id, db_name, db_pass)
...
def api_databases_delete(db_id):
    ...
    _sync_pma_bridge(db_id, row[0], row[1])
...
def api_backups_download(backup_id):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.execute("SELECT file_path FROM backups WHERE id = ?", (backup_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return jsonify({'code': 404, 'msg': '备份不存在'}), 404

    file_path = row[0]
    if not os.path.exists(file_path):
        return jsonify({'code': 404, 'msg': '备份文件不存在'}), 404

    write_log('backup', f'下载备份 {file_path}', request.remote_addr)
...
def api_files_extract():
    data = request.json or {}
    site_id = data.get('site_id')
    archive_path = data.get('path', '')  # 压缩包相对站点根的路径
    delete_after = data.get('delete_after', False)

    if not site_id or not archive_path:
        return jsonify({'code': 400, 'msg': '参数不完整'})
    ...
    full_archive = os.path.join(row[0], archive_path)
    target_dir = os.path.dirname(full_archive)  # 解压到压缩包所在目录

    ok, msg = extract_archive(full_archive, target_dir, delete_after)
    if ok:
        write_log('file_extract', f'解压 {archive_path}', request.remote_addr)
...
def api_admin_files_list():
    ...
    if not real_path.startswith('/'):
        return jsonify({'code': 400, 'msg': '路径格式错误'})
    
    items, err = list_directory(real_path, admin_mode=True)
    if err:
        return jsonify({'code': 400, 'msg': err})

    return jsonify({'code': 0, 'data': items, 'current_path': real_path})

@app.route('/api/admin/files/read', methods=['GET'])
def api_admin_files_read():
    filepath = request.args.get('path', '')
    if not filepath:
        return jsonify({'code': 400, 'msg': '缺少 path 参数'})
    
    real_path = os.path.realpath(filepath)
    content, err = read_file(real_path, admin_mode=True)
    if err:
        return jsonify({'code': 400, 'msg': err})

    return jsonify({'code': 0, 'data': content, 'path': real_path})

@app.route('/api/admin/files/write', methods=['POST'])
def api_admin_files_write():
    data = request.json or {}
    filepath = data.get('path', '')
    content = data.get('content', '')
    ...
    real_path = os.path.realpath(filepath)
    ok, msg = write_file(real_path, content, admin_mode=True)
    if ok:
        write_log('file_edit', f'管理员编辑文件 {real_path}', request.remote_addr)
...
def api_admin_files_delete():
    ...
    real_path = os.path.realpath(filepath)
    ok, msg = delete_file(real_path, admin_mode=True)
    if ok:
        write_log('file_delete', f'管理员删除文件 {real_path}', request.remote_addr)

    return jsonify({'code': 0 if ok else 400, 'msg': msg})

@app.route('/api/admin/files/mkdir', methods=['POST'])
def api_admin_files_mkdir():
    data = request.json or {}
    path = data.get('path', '')
    dirname = data.get('name', '')
    ...
    real_path = os.path.realpath(path)
    ok, msg = create_directory(real_path, dirname, admin_mode=True)
    if ok:
        write_log('file_mkdir', f'管理员创建目录 {real_path}/{dirname}', request.remote_addr)

    return jsonify({'code': 0 if ok else 400, 'msg': msg})

@app.route('/api/admin/files/chmod', methods=['POST'])
def api_admin_files_chmod():
    data = request.json or {}
    filepath = data.get('path', '')
    mode = data.get('mode', 0)
    ...
    real_path = os.path.realpath(filepath)
    ok, msg = chmod_file(real_path, mode, admin_mode=True)
    if ok:
        write_log('file_chmod', f'管理员修改权限 {real_path} → {mode}', request.remote_addr)
...
def api_admin_files_extract():
    ...
    target_dir = data.get('target_dir', '')  # 可选，默认解压到压缩包所在目录
    delete_after = data.get('delete_after', False)
    ...
    real_target = os.path.realpath(target_dir) if target_dir else os.path.dirname(real_archive)

    ok, msg = extract_archive(real_archive, real_target, delete_after, admin_mode=True)
    if ok:
        write_log('file_extract', f'管理员解压 {real_archive} -> {real_target}', request.remote_addr)
...
def api_upgrade_version():
    info = check_latest_version()
    backups = list_backup_versions()
    return jsonify({'code': 0, 'data': info, 'backups': backups})

@app.route('/api/upgrade/upload', methods=['POST'])
def api_upgrade_upload():
    """上传升级包"""
    ...
    if not f.filename.endswith('.zip'):
        return jsonify({'code': 400, 'msg': '只支持 zip 格式安装包'})
    
    upload_path = '/tmp/tpanel_upgrade.zip'
    f.save(upload_path)
    ...
    try:
        with zipfile.ZipFile(upload_path, 'r') as zf:
            ...
            if not has_backend or not has_frontend:
                ...
                return jsonify({'code': 400, 'msg': '安装包格式错误：缺少 backend 或 frontend 目录'})
    except Exception as e:
        os.remove(upload_path)
        return jsonify({'code': 400, 'msg': f'安装包损坏: {str(e)}'})
    
    write_log('upgrade', f'上传升级包: {f.filename}', request.remote_addr)
    return jsonify({'code': 0, 'msg': '安装包已上传，可以开始升级', 'path': upload_path})

@app.route('/api/upgrade/run', methods=['POST'])
def api_upgrade_run():
    """执行升级"""
    zip_path = '/tmp/tpanel_upgrade.zip'
    if not os.path.exists(zip_path):
        return jsonify({'code': 400, 'msg': '请先上传安装包'})
    
    write_log('upgrade', '开始执行升级...', request.remote_addr)
    ok, msg = upgrade_from_zip(zip_path)
    
    if ok:
        write_log('upgrade', f'升级成功: {msg}', request.remote_addr)
        # 升级成功后延迟重启（让API先返回结果）
        _run(['sudo', 'systemctl', 'restart', 'tpanel'], timeout=10)
    else:
        write_log('upgrade', f'升级失败: {msg}', request.remote_addr)
    
    return jsonify({'code': 0 if ok else 400, 'msg': msg})

@app.route('/api/upgrade/rollback', methods=['POST'])
def api_upgrade_rollback():
    """回滚到指定备份版本"""
    data = request.json or {}
    ...
    write_log('upgrade', f'开始回滚到: {backup_file}', request.remote_addr)
    ok, msg = rollback_version(backup_file)
    
    if ok:
        write_log('upgrade', f'回滚成功: {msg}', request.remote_addr)
        _run(['sudo', 'systemctl', 'restart', 'tpanel'], timeout=10)
...
def api_software_list():
    # 支持 ?refresh=1 强制刷新，安装/卸载后调用
    force_refresh = request.args.get('refresh') == '1'
    return jsonify({'code': 0, 'data': list_software(force_refresh=force_refresh)})
...
def api_phpmyadmin_token(db_id):
    ...
    v1.3.40.1: 签 token 时同步 bridge.json，避免新建 db 后未同步
    """
    conn = sqlite3.connect(DB_PATH)
    cur = conn.execute("SELECT name, db_user, db_pass FROM databases WHERE id = ?", (db_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return jsonify({'code': 404, 'msg': '数据库不存在'})
    try:
        _sync_pma_bridge(db_id, row[1], row[2])
    except Exception:
        pass
...
def _sync_pma_bridge(db_id, db_name, db_pass):
    """v1.3.40.1: 把指定 db 凭证写到 /usr/share/phpmyadmin/bridge.json
    bridge.php 启动时读此文件，自动填 PMA_single_signon_*
    """
    import json as _json
    path = '/usr/share/phpmyadmin/bridge.json'
    data = {
        'db_id': db_id,
        'db_user': db_name,
        'db_pass': db_pass,
    ...
    tmp = '/tmp/tpanel_bridge_sync.json.tmp'
    with open(tmp, 'w') as f:
        _json.dump(data, f)
    os.chmod(tmp, 0o644)
    subprocess.run(['sudo', 'mv', tmp, path], check=False)
    subprocess.run(['sudo', 'chmod', '644', path], check=False)
    subprocess.run(['sudo', 'chown', 'www-data:www-data', path], check=False)
...
# v1.3.40.1 修复：动态查 db 所属 site 域名，不再硬编码 zhangpu.tech
    try:
        conn = sqlite3.connect(DB_PATH)
        row = conn.execute(
            "SELECT s.domain, d.db_user, d.db_pass FROM databases d JOIN sites s ON d.site_id = s.id WHERE d.id = ?",
            (db_id,)
        ).fetchone()
        conn.close()
        if not row:
            return redirect('http://127.0.0.1:8888/login?msg=db_not_found')
        site_domain, db_user, db_pass = row[0], row[1], row[2]
    except Exception:
        return redirect('http://127.0.0.1:8888/login?msg=db_lookup_error')
    try:
        _sync_pma_bridge(db_id, db_user, db_pass)
    except Exception:
        pass
    return redirect('https://' + site_domain + '/pma/tpanel-bridge.php?token=' + token + '&db=' + db_id)
...
def api_firewall_status():
    ...
    return jsonify({'code': 0, 'msg': 'ok', 'data': status})

@app.route('/api/firewall/enable', methods=['POST'])
def api_firewall_enable():
    ok, msg = firewall_enable()
    write_log('firewall', '启用防火墙', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})
...
def api_firewall_port():
    ...
    if action == 'open':
        ok, msg = firewall_open_port(port, proto)
    else:
        ok, msg = firewall_close_port(port, proto)
    write_log('firewall', f'{action} 端口 {port}/{proto}', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})

# -------------------------- 面板端口修改 --------------------------
@app.route('/api/panel/port', methods=['GET'])
def api_panel_port():
    port = get_panel_port()
    return jsonify({'code': 0, 'msg': 'ok', 'data': {'port': port}})

@app.route('/api/panel/port', methods=['POST'])
def api_change_panel_port():
    data = request.get_json() or {}
    new_port = data.get('port')
    if not new_port:
        return jsonify({'code': 400, 'msg': '请输入新端口'})
    ok, msg = change_panel_port(new_port)
    write_log('panel_port', f'修改面板端口为 {new_port}', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})
...
def api_backup_settings():
    ...
    return jsonify({'code': 0, 'msg': 'ok', 'data': settings})
...
def api_save_backup_settings():
    ...
    ok, msg = save_backup_settings(enabled, schedule, keep_days, backup_dir)
    write_log('backup', f'配置自动备份: enabled={enabled}, schedule={schedule}', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})

@app.route('/api/backup/run', methods=['POST'])
def api_run_backup():
    ok, msg = run_backup_now()
    write_log('backup', '手动执行备份', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})

@app.route('/api/backup/list', methods=['GET'])
def api_list_backups():
    backups = list_backups()
    return jsonify({'code': 0, 'msg': 'ok', 'data': backups})
...
def api_delete_backup():
    ...
    ok, msg = delete_backup(backup_type, filename)
    write_log('backup', f'删除备份: {backup_type}/{filename}', request.remote_addr)
    return jsonify({'code': 0 if ok else 400, 'msg': msg})
